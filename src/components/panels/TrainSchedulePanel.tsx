...
function NowMarker({ label }: { label: string }) {
  return (
    <li
      className="schedule-now-marker"
      data-schedule-anchor="now-marker"
      aria-hidden
    >
      <span className="schedule-now-marker__line" />
      <span className="schedule-now-marker__label tabular-nums">
        NOW {label}
      </span>
      <span className="schedule-now-marker__line" />
    </li>
...
export function TrainSchedulePanel() {
  ...
  return (
    <div className="schedule-panel">
      <div
        className="schedule-panel__tabs"
        role="tablist"
      ...
      <div className="schedule-panel__scroll" ref={scrollRef}>
        ...
        {status === "error" && (
          <div className="schedule-panel__error">
            ...
            {errorMessage && (
              <p className="schedule-panel__error-detail">{errorMessage}</p>
            )}
            <p className="schedule-panel__error-detail">
              Activate &quot;국토교통부_(TAGO)_열차정보&quot; on data.go.kr,
              then set <code>TAGO_SERVICE_KEY</code> in Vercel for production or{" "}
              <code>VITE_TAGO_SERVICE_KEY</code> in <code>.env</code> for local
              development.
            </p>
            <button
              type="button"
              className="schedule-panel__retry"
        ...
        {status !== "loading" && status !== "error" && rows.length > 0 && (
          <ul className="schedule-list">
            {timeline.map((item) => {
              ...
              const endpoint =
                tab === "depart" ? row.arrPlaceName : row.depPlaceName;
